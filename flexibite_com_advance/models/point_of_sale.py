...
# Take Money Out
    ...
    pos_type = fields.Selection([('drive_through_mode','Drive Through'),
                                 ('dine_in_mode','Dine in'),('online_mode','Online'),('take_away','Take Away')])
...
#             Last 6 month data
    ...
    def test_paid(self):
        ...
        for order in self:
            if order.lines and not order.amount_total:
                continue
            if (not order.lines) or (not float_is_zero((order.amount_total - order.amount_paid), precision_rounding=order.pricelist_id.currency_id.rounding)):
                return False
        return True
    ...
    def create_from_ui(self, orders, draft=False):
        #Credit module
        for ord_emp in orders:
            user_id = self.env['res.users'].browse([ord_emp['data']['user_id']])

            ord_emp['data']['employee_id'] = user_id.employee_id.id
        ...
        for tmp_order in orders_to_save:
            ...
            pos_order = self._process_order(order,draft,existing_order=False)
...
#                 return order_id
        ...
        if not order.get('draft_order') and not draft_order_id:
            ...
            order_proces = {'data':order}

            res = super(pos_order, self)._process_order(order_proces,draft,existing_order=False)
...
#                 'order_time': date_order.split(" ")[1],
    ...
    def _order_fields(self,ui_order):
        ...
        user_id = self.env['res.users'].browse([res['user_id']])
        res['employee_id'] = user_id.employee_id.id
        ...
        if self and self.product_id and self.product_id.product_tmpl_id.id and self.qty > 0:
            ...
            if bom_id:
                ...
                default_dict.update({
                    ...
                    'location_src_id': self.order_id.location_id.id,
                    'location_dest_id': self.order_id.location_id.id,
                })
                ...
                for each in bom_id.bom_line_ids:
                    product_available_qty = self.env['product.product'].browse(each.product_id.id).with_context({'location':self.order_id.location_id.id}).qty_available
...
class pos_session(models.Model):
    ...
    def get_products_category_data(self,flag_config):
        ...
        product_records = self.env['product.product'].search(domain).ids
        if not product_records or len(dummy_products) >= len(product_records):
            domain = [('sale_ok', '=', True),('available_in_pos', '=', True)]
            if self.env.user.has_group('base.group_multi_company'):
                domain += ['|', ('product_tmpl_id.company_id', 'in', user_allowed_company_ids),
                           ('product_tmpl_id.company_id', '=', False)]
            product_records = self.env['product.product'].with_context({'compute_child': False}).search(domain).ids
