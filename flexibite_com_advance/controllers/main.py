...
#             return json.dumps(country_id.read())
    ...
    def load_products(self, **kw):
         ...
         # print(fields,">>>>>>>>>>>>>>>>>>>>product")
         # stock_locfation_idf = eval(kw.get('stock_location_id'))
         if product_ids and fields:
             records = request.env['product.product'].with_context({'compute_child': False}).search_read([('id', 'in', product_ids)], fields)
             template_ids = []
             if records:
                 ...
                 template_fields = ['display_name', 'lst_price', 'standard_price', 'categ_id', 'pos_categ_id', 'taxes_id', 'barcode', 'default_code', 'to_weight', 'uom_id', 'description_sale', 'description', 'tracking', 'qty_available', 'type', 'is_packaging', 'loyalty_point', 'is_dummy_product', 'modifier_line', 'is_combo', 'product_combo_ids', 'return_valid_days', 'non_refundable', 'send_to_kitchen', 'priority', 'lst_price', 'write_date', 'name', 'display_name', 'product_variant_ids', 'product_variant_count']
                 # ['name', 'display_name', 'product_variant_ids','product_variant_count', 'categ_id', 'write_date']
                 template_ids = list(dict.fromkeys(template_ids))
                 # print(template_ids,">>>>>>>>>>>>>>>>>>>>>>>>>.template_ids")
                 product_temp_ids = request.env['product.template'].with_context({'compute_child': False}).search_read([('id', 'in', template_ids)], template_fields)
                 # print(product_temp_ids,">>>>>>>>>>>>>.product_temp_ids")
    ...
    def load_products_template(self, **kw):
        ...
        if product_ids and fields:
            records = request.env['product.template'].with_context({'compute_child': False}).search_read([('id', 'in', product_ids)], fields)
            template_ids = []
            if records:
                for each_rec in records:
                    new_date = each_rec['write_date']
    ...
    def store_data_to_cache(self, **kw):
        ...
        # print(cache_data,"iuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu")
