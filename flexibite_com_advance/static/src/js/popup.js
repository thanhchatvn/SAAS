...
//	                                    	if(prod && selectedOrder.is_sale_product(prod)){
		...
		get_product_image_url: function(product_id){
			return window.location.origin + '/web/binary/image?model=product.product&field=image_1920&id='+product_id;
		},
		renderElement: function(){
			this._super();
	    		...
	    		if(Number(redeem_point_input.val()) <= remaining_loyalty_points){
	    			...
	    			if(amount_to_redeem <= (order.get_due() || order.get_total_with_tax())){
			    		if(self.pos.config.loyalty_journal_id){
				    		...
				    		var loyalty_cashregister = _.find(self.pos.payment_methods, function(cashregister){
                                        ...
                                        if(get_redeem){
                                            ...
                                            if(self.pos.config.enable_journal_id[0]){
                                               ...
                                               for ( var j = 0; j < self.pos.payment_methods.length; j++ ) {
                                                    if(self.pos.payment_methods[j].journal_id[0] === self.pos.config.enable_journal_id[0]){
	    	...
	    	if (self.self_voucher){
	    		...
	    		if (self.pos.config.gift_voucher_journal_id.length > 0){
			        for ( var i = 0; i < self.pos.payment_methods.length; i++ ) {
			            if ( self.pos.payment_methods[i].journal_id[0] === self.pos.config.gift_voucher_journal_id[0] ){
			               cashregister = self.pos.payment_methods[i]
			            }
    				}
    				if (cashregister){
    					if(!vouchers){
...
//        	window.document.body.addEventListener('keydown',self.payment_self.keyboard_keydown_handler);
        ...
        check_redemption_customer: function(){
        	...
        	return rpc.query(params, {async: false}).guardedCatch(function(){
            	self.pos.db.notification('danger',"Connection lost");
            });
    	}
	});
