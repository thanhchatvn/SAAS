...
class account_general_ledger_wiz(models.TransientModel):
    ...
    def print_pdf(self, data):
        ...
        return self.env.ref('flexibite_com_advance.report_general_ledger').report_action(self, data=datas)
...
#         alignment_right.horz = xlwt.Alignment.HORZ_LEFT
        ...
        report_obj = self.env['report.flexibite_com_advance.general_ledger_template']
        data = {}
        data['ids'] = self.env.context.get('active_ids', [])
        data['model'] = self.env.context.get('active_model', 'ir.ui.menu')
        data['form'] = self.read(
