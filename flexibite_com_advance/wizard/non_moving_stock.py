...
class NonMovingStock(models.TransientModel):
    ...
    def print_report(self):
        ...
        if self.report_type == 'pdf':
            if self.env.context.get('send_mail'):
                ...
                template_id = self.env.ref('flexibite_com_advance.non_moving_stock_report')
                ...
                vals_new = {'products': final_non_moving_data}
                return self.env.ref('flexibite_com_advance.non_moving_stock_report').report_action(self, data=vals_new)
    ...
    def send_mail(self):
        ...
        groups_ids = ast.literal_eval(
            self.env['ir.config_parameter'].sudo().get_param('flexibite_com_advance.groups_ids'))
        for each in groups_ids:
            ...
            try:
                template_id = self.env.ref('flexibite_com_advance.send_mail_non_moving_stock_report')
            except Exception:
...
class ResConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        if get_param('flexibite_com_advance.groups_ids'):
            res.update(
                groups_ids=ast.literal_eval(get_param('flexibite_com_advance.groups_ids'))
            )
        return res
    ...
    def set_values(self):
        ...
        set_param('flexibite_com_advance.groups_ids', self.groups_ids.ids)
